  ...
  return (
    <Sheet
      ...
      header={
        ...
        >
          <span className="w-10 h-1 rounded-full bg-neutral-700" aria-hidden="true" />
        </div>
      }
    >
      <div className="flex items-center justify-between px-4 h-12 border-b border-neutral-800 shrink-0 gap-2">
        {inCreate ? (
          <button
            onClick={() => setMode('list')}
            aria-label="Back"
            className="text-neutral-400 hover:text-neutral-200 -ml-2 px-2 h-11 flex items-center gap-1"
          >
            <BackArrowIcon />
            <span className="font-semibold text-neutral-200">New exercise</span>
          </button>
        ) : (
        ...
        <button
          onClick={onClose}
          aria-label="Close"
          className="text-neutral-400 hover:text-neutral-200 w-11 h-11 -mr-2 flex items-center justify-center shrink-0"
        >
          <CloseIcon size={18} />
        </button>
      </div>
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain">
            {presetSubstitutes.length > 0 && !query.trim() && (
              <section>
                <h3 className="sticky top-0 z-10 bg-neutral-950 px-4 pt-3 pb-1 text-[11px] uppercase tracking-wide font-medium text-neutral-400">
                  Substitutes
                </h3>
                <ul>
                  {presetSubstitutes.map((s) => (
                    <li key={`sub-${s.exercise_id}`}>
                      <button
                        ...
                        className="w-full text-left px-4 py-3 flex items-center justify-between hover:bg-neutral-900 transition-colors"
                      >
                        <span className="text-neutral-200">{s.exercise_name}</span>
                        <span className="text-[11px] uppercase tracking-wide text-neutral-400">{s.muscle_group}</span>
                      </button>
                    </li>
                  ))}
                </ul>
            ...
            {Object.keys(grouped).length === 0 ? (
              <div className="px-4 py-8 space-y-3 text-center">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2 rounded border border-dashed border-neutral-800 text-sm text-neutral-300 hover:bg-neutral-900 transition-colors"
                >
                  <PlusIcon />
              ...
              <>
                        ...
                        return (
                          <li key={ex.id}>
                            ...
                            >
                              ...
                              {isCurrent && (
                                <span className="text-[11px] uppercase tracking-wide text-neutral-400">Current</span>
                              )}
                            </button>
                          </li>
                        );
                ...
                <div className="px-4 py-3">
                  <button
                    type="button"
                    onClick={() => openCreate(query.trim())}
                    className="w-full flex items-center justify-center gap-1.5 py-2 text-sm text-neutral-400 hover:text-neutral-200 border border-dashed border-neutral-800 rounded hover:bg-neutral-900 transition-colors"
                  >
                    <PlusIcon />
                    Create new exercise
                  </button>
