...
-- program to match the source spreadsheet's highlighted rows. Run AFTER
-- migrate.sql (which adds the is_main column). Matches rows by routine +
-- exercise name, so it only touches this program. Idempotent — it first clears
-- is_main for the whole program, then sets the highlighted lifts, so re-running
-- (or running after an earlier partial version) always converges to this set.
BEGIN;
...
WHERE re.routine_id = r.id AND re.exercise_id = e.id
  ...
  AND (
    ...
    OR (r.name = $tag$Day A — Squat / Push$tag$ AND e.name = $tag$Weighted Dips$tag$)
    OR (r.name = $tag$Day A — Squat / Push$tag$ AND e.name = $tag$Weighted Pull-Up$tag$)
    OR (r.name = $tag$Day B — Hinge / Row$tag$ AND e.name = $tag$Barbell RDL$tag$)
    OR (r.name = $tag$Day B — Hinge / Row$tag$ AND e.name = $tag$Incline DB Press$tag$)
    OR (r.name = $tag$Day B — Hinge / Row$tag$ AND e.name = $tag$Chest-Supported Row$tag$)
  );
COMMIT;
