...
const exId = ex['Lateral Raise'];
ok(!!exId, 'seed exercise present');
...
await api('PUT', `/api/workouts/${w1.id}`, {
  exercises: [{ exercise_id: exId, notes: null, sets: [
...
await api('PUT', `/api/workouts/${w2.id}`, { exercises: [{ exercise_id: exId, notes: null, sets: [] }] });
...
// Newer still: a template start leaves NULL placeholder rows; completing without ever
// saving keeps them, and they must not count as history either.
const { body: prog } = await api('POST', '/api/programs', {
  name: 'Placeholder Block', total_weeks: 1,
  routines: [{ name: 'Day P', exercises: [{ exercise_id: exId, target_sets: 3, rep_range_low: 8, rep_range_high: 12 }] }],
});
const { body: wp } = await api('POST', '/api/workouts', { routine_id: prog.routines[0].id, date: '2026-09-03' });
ok((wp?.exercises?.[0]?.sets?.length || 0) === 3, 'template start seeded placeholder rows');
await api('POST', `/api/workouts/${wp.id}/complete`);

const { body: w3 } = await api('POST', '/api/workouts', { date: '2026-09-07', exercises: [] });
const { body: prev } = await api('GET', `/api/workouts/last-by-exercise/${exId}?exclude=${w3.id}`);
ok(prev !== null, 'a previous session is returned');
ok(prev?.sets?.length === 2, 'it is the one with numbers, not the empty or placeholder-only newer ones', JSON.stringify(prev));
...
const { body: self } = await api('GET', `/api/workouts/last-by-exercise/${exId}?exclude=${w1.id}`);
